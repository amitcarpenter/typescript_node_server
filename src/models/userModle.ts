...
const user_schema = new Schema<User_Interface>({
  ...
  password: {
    ...
    validate: {
      ...
      message: (props: any) => `${props.value} is not a valid password! Password must contain at least one number, one lowercase letter, one uppercase letter, and one special character.`,
    }
  },
  email: {
    type: String,
